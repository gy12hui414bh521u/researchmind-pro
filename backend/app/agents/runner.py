...
from app.models.task import (
    AgentThoughtEvent, CostInfo, HiTLRequiredEvent,
    SSEEvent, TaskCompletedEvent, TaskFailedEvent,
    TaskProgressEvent, TaskResult,
)
...
async def run_research_stream(
    ...
    try:
        ...
        yield TaskProgressEvent.create(
            task_id=task_id, status="planning", progress=5,
            message="任务已启动，Planner 准备中..."
        ).to_sse()
...
async def resume_after_hitl(
    ...
    try:
        async for event in graph.astream_events(
            resume_data, config=config, version="v2"
        ):
            event_type = event.get("event", "")
            node_name  = event.get("name", "")
            data       = event.get("data", {})
            ...
            if event_type == "on_chain_start" and node_name in _NODE_PROGRESS:
                ...
                if result:
                    ...
                    cost  = _extract_cost(state)
                    yield TaskCompletedEvent.create(
                        task_id=task_id, result=result, cost=cost,
                    ).to_sse()
                    return

    except Exception as e:
        yield TaskFailedEvent.create(
            task_id=task_id, error_code="RESUME_ERROR", message=str(e),
        ).to_sse()
...
async def save_task_to_memory(
    user_id: str, task_id: str, query: str, result
) -> None:
    """任务完成后将摘要写入长期记忆"""
    try:
        from app.memory.store import get_memory_store
        ...
        await memory.save_task_summary(
            ...
            query=query, summary=summary, keywords=keywords,
        )
    except Exception:
        pass   # 记忆写入失败不影响主流程
