...
from app.agents.llm_factory import (
    get_analyst_llm, get_critic_llm, get_planner_llm,
    get_research_llm, get_writer_llm,
)
from app.agents.prompts import (
    ...
    WRITER_SYSTEM, WRITER_USER, WRITER_CRITIC_FEEDBACK,
)
from app.agents.tools import RESEARCH_TOOLS
from app.config import settings
from app.models.agent import (
    AnalystOutput, CriticEvaluation, CriticOutput,
    PlannerOutput, ResearchOutput, WriterOutput,
)
...
async def planner_node(state: dict) -> PlannerOutput:
    ...
    try:
        ...
        memory_context = await get_memory_store().build_memory_context(
            user_id=state["user_id"], query=state["user_query"],
        )
    except Exception:
        pass

    user_prompt = PLANNER_USER.format(
        query=state["user_query"], depth=state["task_depth"],
    )
    if memory_context:
...
async def research_node(state: dict) -> ResearchOutput:
    ...
    pending = [
        st for st in state.get("sub_tasks", [])
        if st.agent == "research" and st.status == "pending"
    ]
    if not pending:
    ...
    existing_summary = (
        f"已检索 {len(existing)} 条资料，涉及: " +
        "、".join({r.source for r in existing[:5]})
        if existing else "无"
    )
...
async def writer_node(state: dict) -> WriterOutput:
    ...
    sources_summary = "\n".join(
        f"- {r.source}: {r.text[:100]}..." for r in results[:8]
    ) or "无参考来源"

    # Critic 反馈（如有）
    feedback = state.get("critic_feedback")
    critic_section = (
        WRITER_CRITIC_FEEDBACK.format(feedback=feedback)
        if feedback else ""
    )
...
async def critic_node(state: dict) -> CriticOutput:
    ...
    raw = _parse_json_safe(response.content, {
        "score": 0.76, "faithfulness": 0.76, "completeness": 0.76,
        "coherence": 0.76, "actionability": 0.76,
        "passed": True, "feedback": "解析失败，默认通过", "flags": [],
    })
