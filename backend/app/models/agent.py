...
from app.models.task import SearchResult, SubTask, TaskPlan, TaskResult, CostInfo
...
class ResearchState(TypedDict):
    ...
    # ── Analyst 输出 ──────────────────────────────────────────────────
    structured_data: Optional[dict[str, Any]]   # DataFrame 序列化为 dict
    sql_query:       Optional[str]              # 记录执行的 SQL

    # ── Writer/Critic 迭代 ────────────────────────────────────────────
    draft_report:    Optional[str]
    quality_score:   Optional[float]
    critic_feedback: Optional[str]   # Critic 的改进建议，注入下一轮 Writer
    iteration_count: int             # 防无限循环，最大 MAX_CRITIC_ITERATIONS

    # ── HiTL 控制 ─────────────────────────────────────────────────────
    hitl_required:  bool
    human_approved: bool
    human_feedback: Optional[str]   # 用户审批时的修改意见
