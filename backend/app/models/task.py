...
class TaskCreate(BaseModel):
    ...
    context: dict[str, Any] = Field(default={}, description="补充上下文，如领域、语言偏好等")
...
class TaskApprove(BaseModel):
    ...
    modifications: dict[str, Any] = Field(
        default={}, description="action=modify 时传入修改内容，如 {plan: {...}}"
    )
    comment: str = ""
...
class ToolResultEvent(SSEEvent):
    ...
    def create(cls, task_id: str, agent: str, tool: str, result: Any) -> ToolResultEvent:
        return cls(
            task_id=task_id, data={"agent": agent, "tool": tool, "result": str(result)[:500]}
        )
...
class HiTLRequiredEvent(SSEEvent):
    ...
    def create(cls, task_id: str, plan: dict, reason: str, timeout: int = 300) -> HiTLRequiredEvent:
        return cls(
            task_id=task_id, data={"plan": plan, "reason": reason, "timeout_seconds": timeout}
        )
...
class TaskProgressEvent(SSEEvent):
    ...
    @classmethod
    def create(
        cls, task_id: str, status: str, progress: int, message: str = ""
    ) -> TaskProgressEvent:
        return cls(
            task_id=task_id, data={"status": status, "progress": progress, "message": message}
        )
