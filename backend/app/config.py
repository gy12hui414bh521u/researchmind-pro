...
class LLMSettings(BaseSettings):
    ...
    def available_providers(self) -> list[str]:
        p = []
        if self.has_deepseek:  p.append("deepseek")
        if self.has_qwen:      p.append("qwen")
        if self.has_openai:    p.append("openai")
        if self.has_anthropic: p.append("anthropic")
        return p
    ...
    def get_llm_config(self, spec: str) -> dict:
        ...
        if provider == "deepseek":
            return {**base, "model": model, "api_key": self.deepseek_api_key,
                    "base_url": self.deepseek_base_url}

        elif provider == "qwen":
            return {**base, "model": model, "api_key": self.qwen_api_key,
            ...
            return {**base, "model": model, "api_key": self.anthropic_api_key,
                    "_provider": "anthropic"}
    ...
    def get_embedding_config(self) -> dict:
        ...
        """
        def _qwen_cfg() -> dict:
            return {"provider": "qwen", "model": self.qwen_embedding_model,
                    "api_key": self.qwen_api_key, "base_url": self.qwen_base_url,
                    "dimensions": self.embedding_dimensions}

        def _openai_cfg() -> dict:
            return {"provider": "openai", "model": self.openai_embedding_model,
                    "api_key": self.openai_api_key, "dimensions": 1536}
        ...
        raise ValueError(
            "无法初始化 Embedding：请配置 QWEN_API_KEY 或 OPENAI_API_KEY。"
        )
...
class Settings(
    ...
    def validate_startup(self) -> list[str]:
        ...
        if not self.available_providers:
            warnings.append(
                "❌ 【严重】未配置任何 LLM API Key！"
                "请配置 DEEPSEEK_API_KEY 或 QWEN_API_KEY。"
            )
